 ...
 */
import { BarChart3, Check, Copy, Download, Expand, Trash2 } from 'lucide-react'
...
export function MessageFooter({
  ...
  return (
    <div className={cn(!hideMetadata && 'mt-2 pt-1.5 border-t border-border/30', className)}>
        ...
        <div className={cn('flex items-center gap-0.5', hideMetadata && 'ml-auto')}>
          {telemetry && (
            <Button
              ...
              size="sm"
              className={cn(
                'h-6 px-2 text-[11px] opacity-0 group-hover:opacity-70 hover:opacity-100! transition-opacity',
                showTelemetry && 'opacity-100',
              )}
              onClick={() => setShowTelemetry(prev => !prev)}
              title={showTelemetry ? '隐藏透明化面板' : '查看透明化面板'}
            >
